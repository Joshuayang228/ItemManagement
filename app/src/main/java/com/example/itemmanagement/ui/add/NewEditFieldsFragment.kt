 ...
 */
class NewEditFieldsFragment : BottomSheetDialogFragment() {
    ...
    companion object {
        ...
        ): NewEditFieldsFragment {
            return NewEditFieldsFragment().apply {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        Log.d("NewEditFieldsFragment", "=== 编辑字段Fragment创建 ===")
        Log.d("NewEditFieldsFragment", "购物模式: $isShoppingMode")
        Log.d("NewEditFieldsFragment", "标签页: $tabs")
        
        // 记录当前选中的字段
        val currentSelectedFields = baseViewModel.selectedFields.value ?: emptySet()
        Log.d("NewEditFieldsFragment", "当前已选中字段数量: ${currentSelectedFields.size}")
        currentSelectedFields.sortedBy { it.order }.forEach { field ->
            Log.d("NewEditFieldsFragment", "已选中字段: ${field.name} (组: ${field.group}, 顺序: ${field.order})")
        }
    ...
    private fun setupViewPager() {
        ...
        binding.viewPager.registerOnPageChangeCallback(object : ViewPager2.OnPageChangeCallback() {
            override fun onPageSelected(position: Int) {
                ...
                Log.d("NewEditFieldsFragment", "=== 切换到标签页: $tabName (位置: $position) ===")
                
                // 记录当前选中字段状态
                val currentSelectedFields = baseViewModel.selectedFields.value ?: emptySet()
                Log.d("NewEditFieldsFragment", "当前选中字段数量: ${currentSelectedFields.size}")
                currentSelectedFields.sortedBy { it.order }.forEach { field ->
                    Log.d("NewEditFieldsFragment", "选中字段: ${field.name} (顺序: ${field.order})")
                }
            }
        })
    }
    ...
    private inner class FieldsPagerAdapter : FragmentStateAdapter(this) {
        ...
        override fun createFragment(position: Int): Fragment {
            ...
            // 记录每个标签页的字段内容和顺序
            Log.d("NewEditFieldsFragment", "=== 创建标签页: $tabName ===")
            Log.d("NewEditFieldsFragment", "字段数量: ${fields.size}")
            
            // 检查order的连续性
            val orders = fields.map { it.order }.sorted()
            Log.d("NewEditFieldsFragment", "所有order值: $orders")
            
            // 检查是否有重复的order
            val duplicateOrders = orders.groupBy { it }.filter { it.value.size > 1 }
            if (duplicateOrders.isNotEmpty()) {
                Log.w("NewEditFieldsFragment", "发现重复的order: $duplicateOrders")
            }
            
            // 按order排序后记录
            val sortedFieldsForLog = fields.sortedBy { it.order }
            sortedFieldsForLog.forEachIndexed { index, field ->
                Log.d("NewEditFieldsFragment", "[$index] ${field.name} (组: ${field.group}, 顺序: ${field.order}, 选中: ${field.isSelected})")
            }
            
            // 记录实际传递给NewFieldListFragment的字段顺序
            Log.d("NewEditFieldsFragment", "传递给NewFieldListFragment的字段顺序:")
            fields.forEachIndexed { index, field ->
                Log.d("NewEditFieldsFragment", "原始[$index] ${field.name} (order: ${field.order})")
            }

            // 确保字段按order排序后传递给NewFieldListFragment
            val sortedFields = fields.sortedBy { it.order }
            Log.d("NewEditFieldsFragment", "最终排序后传递的字段:")
            sortedFields.forEachIndexed { index, field ->
                Log.d("NewEditFieldsFragment", "最终[$index] ${field.name} (order: ${field.order})")
            }
            
            return NewFieldListFragment.newInstance(sortedFields) { field, isSelected ->
                Log.d("NewEditFieldsFragment", "字段选择变化: ${field.name} -> $isSelected")
    ...
    private fun getAllFields(): List<Field> {
        Log.d("NewEditFieldsFragment", "=== 构建全部字段列表 ===")
        val selectedFields = baseViewModel.selectedFields.value ?: emptySet()
        Log.d("NewEditFieldsFragment", "ViewModel中选中字段数量: ${selectedFields.size}")
        selectedFields.forEach { field ->
            Log.d("NewEditFieldsFragment", "ViewModel选中字段: ${field.name} (order: ${field.order})")
        }
        ...
        selectedFields.forEach { selectedField ->
            if (allFieldNames.contains(selectedField.name)) {
                ...
                Log.d("NewEditFieldsFragment", "添加已选中字段: ${selectedField.name} (order: ${selectedField.order})")
            }
        }
        
        // 获取已使用的order值，避免冲突
        val usedOrders = selectedFields.map { it.order }.toSet()
        Log.d("NewEditFieldsFragment", "已使用的order值: $usedOrders")
        ...
        allFieldNames.forEach { fieldName ->
            if (!selectedFields.any { it.name == fieldName }) {
                ...
                while (usedOrders.contains(defaultOrder)) {
                    ...
                    Log.d("NewEditFieldsFragment", "字段 $fieldName 的order冲突，调整为: $defaultOrder")
                }
                
                commonFields.add(Field(group, fieldName, false, defaultOrder))
                Log.d("NewEditFieldsFragment", "添加未选中字段: $fieldName (order: $defaultOrder)")
            }
        }
        
        Log.d("NewEditFieldsFragment", "创建的通用字段数量: ${commonFields.size}")
        commonFields.sortedBy { it.order }.forEachIndexed { index, field ->
            Log.d("NewEditFieldsFragment", "通用字段[$index]: ${field.name} (order: ${field.order}, selected: ${field.isSelected})")
        }
        
        // 如果是购物模式，添加购物专用字段
        val finalFields = if (isShoppingMode) {
            val shoppingFields = getShoppingFields()
            Log.d("NewEditFieldsFragment", "购物字段数量: ${shoppingFields.size}")
            commonFields + shoppingFields
        } else {
            commonFields
        }
        
        Log.d("NewEditFieldsFragment", "最终字段列表数量: ${finalFields.size}")
        return finalFields
    }
    ...
    private fun selectAllFields() {
        Log.d("NewEditFieldsFragment", "=== 开始全选字段 ===")
        
        try {
            // 获取所有字段
            val allFields = getAllFields()
            Log.d("NewEditFieldsFragment", "获取到所有字段数量: ${allFields.size}")
            
            if (allFields.isEmpty()) {
                Log.w("NewEditFieldsFragment", "没有可选择的字段")
                return
            }
            ...
            Log.d("NewEditFieldsFragment", "设置选中字段数量: ${allFields.size}")
            ...
            binding.root.post {
                try {
                    ...
                    Log.d("NewEditFieldsFragment", "全选完成，UI已刷新")
                } catch (e: Exception) {
                    Log.e("NewEditFieldsFragment", "刷新UI时出错: ${e.message}")
                    // 确保按钮状态恢复
                    binding.selectAllButton.isEnabled = true
                    binding.selectAllButton.text = "全选"
                }
            ...
            Log.e("NewEditFieldsFragment", "全选字段时出错: ${e.message}")
            // 确保按钮状态恢复
            binding.selectAllButton.isEnabled = true
            binding.selectAllButton.text = "全选"
