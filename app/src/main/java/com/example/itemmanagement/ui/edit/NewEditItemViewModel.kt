...
class NewEditItemViewModel(
    ...
    init {
        Log.d("NewEditItemViewModel", "=== 初始化编辑ViewModel，物品ID: $itemId ===")
        ...
        if (cacheViewModel.hasEditItemCache(itemId)) {
            Log.d("NewEditItemViewModel", "发现编辑缓存，从缓存恢复数据")
            loadFromCache()
        } else {
            Log.d("NewEditItemViewModel", "没有编辑缓存，加载物品数据进行编辑")
            // 如果没有缓存，加载物品数据进行编辑
            loadItemForEdit()
        }
        
        Log.d("NewEditItemViewModel", "编辑ViewModel初始化完成，当前fieldValues: $fieldValues")
    }
    ...
    private fun loadItemForEdit() {
        viewModelScope.launch {
            try {
                // 从数据库加载真实的物品数据
                Log.d("NewEditItemViewModel", "正在加载物品 ID: $itemId")
                val item = repository.getItemById(itemId)
                if (item != null) {
                    Log.d("NewEditItemViewModel", "找到物品: ${item.name}, 数量: ${item.quantity}")
                    loadItemData(item)
                } else {
                    Log.e("NewEditItemViewModel", "找不到物品 ID: $itemId")
    ...
    private fun loadItemData(item: Item) {
        Log.d("NewEditItemViewModel", "开始加载物品数据: ${item.name}")
        ...
        Log.d("NewEditItemViewModel", "设置名称: ${item.name}")
        ...
        item.rating?.let { 
            Log.d("NewEditItemViewModel", "处理评分字段: $it")
            saveFieldValue("评分", it.toString())
            fieldsToShow.add(Field("数字类", "评分", true, getEditModeOrder("评分")))
            Log.d("NewEditItemViewModel", "评分字段已保存到fieldValues: ${getFieldValue("评分")}")
        } ?: Log.d("NewEditItemViewModel", "评分字段为null")
        ...
        item.season?.let { seasonString ->
            Log.d("NewEditItemViewModel", "处理季节字段: '$seasonString'")
            if (seasonString.isNotBlank() && seasonString != "未指定") {
                ...
                Log.d("NewEditItemViewModel", "季节字段已转换并保存: 原始='$seasonString' -> Set=$seasonSet")
            } else {
                Log.d("NewEditItemViewModel", "季节字段被跳过，值为空或未指定")
            }
        } ?: Log.d("NewEditItemViewModel", "季节字段为null")
        ...
        item.shelfLife?.let { if (it > 0) {
            ...
            Log.d("NewEditItemViewModel", "保质期已设置: $value $unit (原始天数: $it)")
        }}
        ...
        item.warrantyPeriod?.let { if (it > 0) {
            ...
            Log.d("NewEditItemViewModel", "保修期已设置: $value $unit (原始天数: $it)")
        }}
        ...
        // 设置字段选择状态
        Log.d("NewEditItemViewModel", "设置字段选择状态，总共 ${fieldsToShow.size} 个字段")
        fieldsToShow.forEach { field ->
            updateFieldSelection(field, field.isSelected)
            Log.d("NewEditItemViewModel", "设置字段: ${field.name}")
        }
        
        Log.d("NewEditItemViewModel", "当前fieldValues内容: $fieldValues")
        
        // 加载照片数据
        if (item.photos.isNotEmpty()) {
            Log.d("NewEditItemViewModel", "加载 ${item.photos.size} 张照片")
            val photoUris = item.photos.map { photo ->
                Uri.parse(photo.uri)
            }
            _photoUris.value = photoUris
            Log.d("NewEditItemViewModel", "照片URI已设置: $photoUris")
        } else {
            Log.d("NewEditItemViewModel", "没有照片数据需要加载")
    ...
    private fun buildItemFromFields(): Item {
        return Item(
            ...
            rating = (getFieldValue("评分")?.toString())?.toDoubleOrNull().also {
                Log.d("NewEditItemViewModel", "buildItemFromFields - 评分: fieldValue=${getFieldValue("评分")}, parsed=$it")
            },
            ...
            season = getFieldValue("季节")?.toString().also {
                Log.d("NewEditItemViewModel", "buildItemFromFields - 季节: fieldValue=${getFieldValue("季节")}, result=$it")
            },
