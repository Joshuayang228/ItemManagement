...
class NewAddItemViewModel(
    ...
    init {
        ...
        if (cacheViewModel.hasAddItemCache()) {
            Log.d("NewAddItemViewModel", "发现添加缓存，从缓存恢复数据")
            loadFromCache()
        } else {
            Log.d("NewAddItemViewModel", "没有添加缓存，初始化默认字段")
            // 如果没有缓存，初始化默认字段
            initializeDefaultFields()
        }
        
        Log.d("NewAddItemViewModel", "添加ViewModel初始化完成，当前fieldValues: $fieldValues")
    }
    ...
    override fun loadDataFromCache() {
        Log.d("NewAddItemViewModel", "开始从缓存加载数据")
        val cache = cacheViewModel.getAddItemCache()
        Log.d("NewAddItemViewModel", "缓存内容: fieldValues=${cache.fieldValues}, selectedFields=${cache.selectedFields}")
        // 类型安全的缓存加载，不需要类型转换
        ...
        Log.d("NewAddItemViewModel", "缓存加载完成，当前fieldValues: $fieldValues")
    }
    ...
    override suspend fun saveOrUpdateItem() {
        Log.d("NewAddItemViewModel", "开始保存物品数据")
        
        // 详细记录所有字段值
        Log.d("NewAddItemViewModel", "=== 保存前字段值详情 ===")
        fieldValues.forEach { (key, value) ->
            Log.d("NewAddItemViewModel", "字段 '$key': $value (${value?.javaClass?.simpleName})")
        }
        
        // 验证数据
        val item = buildItemFromFields()
        Log.d("NewAddItemViewModel", "构建的Item对象: $item")
        
        val (isValid, errorMessage) = validateItem(item)
        
        if (!isValid) {
            Log.e("NewAddItemViewModel", "数据验证失败: $errorMessage")
