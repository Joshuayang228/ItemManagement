 ...
 * 3. 与编辑模式完全隔离的数据
 */
class NewAddItemFragment : NewBaseItemFragment<NewAddItemViewModel>() {

    // 获取添加物品专用的 ViewModel
    override val viewModel: NewAddItemViewModel by viewModels {
    ...
    private fun showEditFieldsDialog() {
        ...
        val editFieldsFragment = NewEditFieldsFragment.newInstance(viewModel, false)
    ...
    private fun clearAllFields() {
        ...
        if (!isAdded || _binding == null) {
            android.util.Log.w("NewAddItemFragment", "尝试在Fragment销毁后清空字段，跳过操作")
            return
        }
    ...
    private fun navigateToNewAddItem() {
        try {
            ...
            android.util.Log.w("NewAddItemFragment", "导航失败，使用清空页面方式: ${e.message}")
            if (isAdded && _binding != null) {
                // 先显示"正在准备新的添加页面"的提示
